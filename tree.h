...
struct Tree {
	...
	void addChild(int evalScore, vector<Coord> move) { children.emplace_back(make_shared<Tree>(evalScore, move[0], move[1], this)); }
	int findMaxEval() {
		if (children.size() == 0) return evalScore;
		int maxscore = -1000;
		for (size_t i = 0; i < children.size(); ++i) {
			int calcscore = findMaxEval(children[i]);
	...
	int findMinEval() {
		...
		int minscore = 1000;
		for (size_t i = 0; i < children.size(); ++i) {
			int calcscore = findMinEval(children[i]);
