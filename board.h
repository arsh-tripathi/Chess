...
class Board {
  ...
  void undo() {
    ...
    theBoard[undoInfo.end.x()][undoInfo.end.y()]->move(
        *theBoard[undoInfo.start.x()][undoInfo.start.y()], &undoInfo);
  }
  ...
  std::shared_ptr<TextDisplay> td =
      nullptr; // initializabled in some init function
  // GraphicsDisplay* gd;

  bool isWhiteMove = true;
  State status =
      State::Normal;   // normal, check, checkmate, statemate, resign, invalid
...
public:
  ...
  Board(Board &other) { // shouldn't need for now since we have undo() . may
                        // need for AI
  }
  Board(Board &&other) { // shouldn't need for now  since we have undo() . may
                         // need for AI
  }
  Board &operator=(Board &other) { // shouldn't need for now  since we have
  ...
  void placePiece(Colour colour, Coord coord, PieceType pt) {
    ...
    if (pt == PieceType::Pawn) {
      ...
      cerr << "ERROR: Board.h => void placePiece(...) => invalid PieceType"
           << endl;
    ...
    if (colour == Colour::White) {
      whitePieces.emplace_back(std::move(
          nPiece)); //!!! make sure this is calling move from <utility>
    } else if (colour == Colour::Black) {
      blackPieces.emplace_back(std::move(
          nPiece)); //!!! make sure this is calling move from <utility>
    } else {
      cerr << "ERROR: Board.h => void placePiece(...) => invalid Colour"
           << endl;
      // maybe throw exception
    }
  }
  ...
  void setupDefaultBoard() {
    ...
    // Knights
    unique_ptr<Piece> wknight1 =
        make_unique<Knight>(Coord{1, 0}, Colour::White);
    theBoard[1][0]->setPiece(wknight1.get());
    whitePieces.emplace_back(std::move(wknight1));
    unique_ptr<Piece> wknight2 =
        make_unique<Knight>(Coord{6, 0}, Colour::White);
    theBoard[6][0]->setPiece(wknight2.get());
    whitePieces.emplace_back(std::move(wknight2));
    // Bishops
    unique_ptr<Piece> wbishop1 =
        make_unique<Bishop>(Coord{2, 0}, Colour::White);
    theBoard[2][0]->setPiece(wbishop1.get());
    whitePieces.emplace_back(std::move(wbishop1));
    unique_ptr<Piece> wbishop2 =
        make_unique<Bishop>(Coord{5, 0}, Colour::White);
    ...
    // Knights
    unique_ptr<Piece> bknight1 =
        make_unique<Knight>(Coord{1, 7}, Colour::Black);
    theBoard[1][7]->setPiece(bknight1.get());
    blackPieces.emplace_back(std::move(bknight1));
    unique_ptr<Piece> bknight2 =
        make_unique<Knight>(Coord{6, 7}, Colour::Black);
    theBoard[6][7]->setPiece(bknight2.get());
    blackPieces.emplace_back(std::move(bknight2));
    // Bishops
    unique_ptr<Piece> bbishop1 =
        make_unique<Bishop>(Coord{2, 7}, Colour::Black);
    theBoard[2][7]->setPiece(bbishop1.get());
    blackPieces.emplace_back(std::move(bbishop1));
    unique_ptr<Piece> bbishop2 =
        make_unique<Bishop>(Coord{5, 7}, Colour::Black);
  ...
  bool isWhiteTurn() { return isWhiteMove; }

  void toggleTurn() { isWhiteMove = !isWhiteMove; }
  ...
  bool isPossibleMove(Coord curr, Coord dest) {
    ...
    if (theBoard[curr.x()][curr.y()]->getPiece()->getColour() !=
        currPlayerColour) {
...
private:
  ...
  bool enPassentMove(Coord curr, Coord dest, const Coord capturedPiece) {
    // move the pawn
    theBoard[curr.x()][curr.y()]->move(*theBoard[dest.x()][dest.y()], &undoInfo,
                                       &status);
    isWhiteMove = !isWhiteMove;
    ...
    if (status == State::Invalid) {
      undo(); // !!! fix undo
      cerr << "You entered into an invalid state from that ENPASSANT move"
           << endl;
      return false;
    }
    ...
    theBoard[capturedPiece.x()][capturedPiece.y()]->notifyDisplayObservers(
        *theBoard[0][0]);
    theBoard[curr.x()][curr.y()]->notifyDisplayObservers(
        *theBoard[dest.x()][dest.y()]);
    return true;
  }
  ...
  bool promotion(Coord curr, Coord dest) {
    ...
    if (status == State::Invalid) {
      undo(); // !!! fix undo
      cerr << "You entered into an invalid state from that PROMOTION move"
           << endl;
      return false;
    }
...
public:
  ...
  bool move(Coord curr, Coord dest) {
    ...
    if (theBoard[curr.x()][curr.y()]->getPiece()->getPieceType() ==
        PieceType::Pawn) {
      if ((dest - curr).x() == 0) { // move forward in straight line
        ...
        if (!(theBoard[dest.x()][dest.y()]
                  ->getPiece())) { // no piece diagonally must be en passant
          ...
          if (theBoard[dest.x()][dest.y() + difference]
                  ->getPiece()
                  ->getPieceType() != PieceType::Pawn)
          ...
          return enPassentMove(curr, dest,
                               Coord{dest.x(), dest.y() + difference});
        }
      }
    }
    ...
    vector<vector<Coord>> newCellObs =
        theBoard[dest.x()][dest.y()]->getPiece()->possibleMoves();
    for (size_t r = 0; r < newCellObs.size(); ++r) {
      for (size_t c = 0; c < newCellObs[r].size(); ++c) {
        shared_ptr<Cell> targetCell =
            theBoard[newCellObs[r][c].x()][newCellObs[r][c].y()];
        ...
        if (!targetCell ||
            !targetCell->getPiece()) // what sthe point of this check
        {
          continue;
        }
        // check if the attacked piece is a king of opposite colour of the moved
        // piece toggleTurn(); // doesnt work NOT NECESSARILY ATTACKED THO COULD
        // BE BLOCKED ??
            ...
            targetCell->getPiece()->getColour() == col) // added condition
        {
          ...
          if (col == Colour::White) {
            piecesAttackingWhiteKing.emplace_back(
                theBoard[dest.x()][dest.y()]); // changed
          } else {
  ...
  void updateState() {
    if (isWhiteMove) {
      ...
      for (size_t i = 0; i < piecesAttackingBlackKing.size(); ++i) {
        if (isPossibleMove(piecesAttackingBlackKing[i]->getCoordinate(),
                           blackKing->getCoordinate())) {
      ...
      for (size_t i = 0; i < piecesAttackingWhiteKing.size(); ++i) {
        if (isPossibleMove(piecesAttackingWhiteKing[i]->getCoordinate(),
                           whiteKing->getCoordinate())) {
          status = State::Invalid;
          return;
        }
      }
...
public:
  ...
  std::vector<std::shared_ptr<Cell>> possibleMoves(Coord curr) {
    ...
    vector<vector<Coord>> possibleMovesUnfiltered =
        currCellPtr->getPiece()->possibleMoves();
  ...
  bool singlePathBlockCheck(Coord curr, Coord dest) {
    ...
    for (size_t r = 0; r < pmoves.size(); ++r) {
      for (size_t c = 0; c < pmoves[r].size(); ++c) {
        if (theBoard[pmoves[r][c].x()][pmoves[r][c].y()]->getCoordinate() ==
            dest)
          return true; // found path yay
        else if (theBoard[pmoves[r][c].x()][pmoves[r][c].y()]->getPiece() !=
                 nullptr)
